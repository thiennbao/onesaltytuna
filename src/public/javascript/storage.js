...
function loadPage(pg) {
    ...
    .then(data => {
        var start = data.indexOf('<h2',data.indexOf('<span id="storage-list-start"></span>'))
        var end = data.indexOf('<span id="storage-list-end"></span>')
        var list = data.slice(start, end)
        var check = list.indexOf('<div class="user-info">')
        if (check != -1) {
            $('#storage-list').html(list)
            $('#storage-page').text(pg)
            fillTable()
        } else {
            page--
        }
